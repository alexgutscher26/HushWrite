...
- [x] [WIN] [SEC] Memory-safe clipboard clear on lock — On WM_WTSSESSION_CHANGE with WTS_SESSION_LOCK, zero out the clipboard if it contains a HushWrite-set value using EmptyClipboard() followed by CloseClipboard().
- [x] [WIN] [INFRA] ARM64 Windows build — Add an aarch64-pc-windows-msvc target to the release CI matrix for Snapdragon X Elite / Surface Pro devices.
...
- [x] [FEAT] Post-processing rule priority ordering — Let users drag-and-drop the order in which enhancement rules execute. Persist order in registry as an ordered list of rule IDs. Implemented as `adapters/rules/order.rs` (RuleId + canonical RULES table + resolve_rule_order merge), the `enhance.rule_order` setting, and drag-and-drop in the RulePreviewSandbox. Whitespace is pinned first and the terminal stop stays last; new rules added later slot into their canonical positions in a stored order.
- [x] [FEAT] Rule preview sandbox — A text area in settings where users can paste raw transcript text and see how each enabled rule transforms it in real time, rule-by-rule, with a diff view. Implemented as `RulePreviewSandbox.tsx` (Settings > Output & Typing) over the `preview_rule_pipeline` IPC command, which runs the production RuleEnhancer pass (`enhance_with_trace`) under the live settings so the preview cannot drift from real sessions.
- [x] [FEAT] Profanity filter (opt-in) — Optional bleep/asterisk replacement of profanity, configurable per app profile (e.g. enable in Slack, disable in personal notes).
- [x] [PERF] Rules pipeline caching — Cache the compiled regex patterns for filler words, corrections, and punctuation rules across sessions so they are not recompiled on every chunk. (No regex crate is used; cached the actual per-chunk work — sorted filler/command/ordinal/TLD tables and a match-probe early-out in the word replacer.)
