...
fn main() {
    ...
    // HUSHWRITE PATCH: restore optimization on MSVC.
    //
    // cmake-rs rewrites CMAKE_{C,CXX}_FLAGS_RELEASE with cc-crate boilerplate
    // (`/utf-8 -nologo -MD -Brepro -W0`), which REPLACES CMake's default
    // `/O2 /Ob2 /DNDEBUG` instead of extending it. With the Visual Studio
    // generator MSVC then compiles ggml with no optimization flag at all
    // (/Od default). Measured on a Ryzen 7 7700X: small-q5_1 on the 11 s JFK
    // sample decodes in ~44 s unoptimized vs ~3.3 s with /O2 — a 10x+
    // slowdown invisible in every log. Defining the vars here restores /O2
    // AND stops cmake-rs from injecting its flag-less version (it only adds
    // its own when the var is undefined). A CMAKE_*_FLAGS_RELEASE env var
    // still wins: the env-forwarding loop below appends a later -D, and CMake
    // takes the last one. Non-MSVC targets keep upstream behavior untouched.
    if target.contains("msvc") {
        ...
        // Debug builds map to RelWithDebInfo and hit the same flag rewrite,
        // so `cargo run`/`tauri dev` get the identical fix (mirrors CMake's
        // RelWithDebInfo defaults: /O2 /Ob1 /DNDEBUG /Zi).
        config.define(
            "CMAKE_C_FLAGS_RELWITHDEBINFO",
            "/MD /O2 /Ob1 /DNDEBUG /Zi /utf-8 -nologo -Brepro -W0",
        );
        config.define(
            "CMAKE_CXX_FLAGS_RELWITHDEBINFO",
            "/MD /O2 /Ob1 /DNDEBUG /Zi /utf-8 -nologo -Brepro -W0",
        );
        // ggml's CMake only adds /arch:AVX2 when GGML_AVX2 is explicitly ON,
        // and GGML_NATIVE=ON on MSVC emits /arch:AVX512 (bad codegen for Zen
        // 4/5 in ggml's hand-written kernels and unusable on pre-Zen4 AMD).
        // Bake the wide-compat validated config; env vars still override.
