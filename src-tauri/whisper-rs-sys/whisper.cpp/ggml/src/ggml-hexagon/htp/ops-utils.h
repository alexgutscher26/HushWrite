...
static inline void htp_dump_int8_line(char * pref, const int8_t * x, int n) {
    ...
    int written      = snprintf(p, p_end - p, "%s: ", pref);
    if (written < 0 || written >= (int)(p_end - p)) {
        FARF(HIGH, "%s\n", str);
        return;
    }
    p += written;
    for (int i = 0; i < n && p < p_end; i++) {
        written = snprintf(p, p_end - p, "%d, ", x[i]);
        if (written < 0 || written >= (int)(p_end - p)) {
            break;
        }
        p += written;
...
static inline void htp_dump_int32_line(char * pref, const int32_t * x, uint32_t n) {
    ...
    p += written;
    for (int i = 0; i < n && p < p_end; i++) {
        written = snprintf(p, p_end - p, "%d, ", (int) x[i]);
        if (written < 0 || written >= (int)(p_end - p)) {
            break;
        }
        p += written;
...
static inline void htp_dump_fp16_line(char * pref, const __fp16 * x, uint32_t n) {
    ...
    p += written;
    for (int i = 0; i < n && p < p_end; i++) {
        written = snprintf(p, p_end - p, "%.6f, ", (float) x[i]);
        if (written < 0 || written >= (int)(p_end - p)) {
            break;
        }
        p += written;
...
static inline void htp_dump_fp32_line(char * pref, const float * x, uint32_t n) {
    ...
    p += written;
    for (int i = 0; i < n && p < p_end; i++) {
        written = snprintf(p, p_end - p, "%.6f, ", x[i]);
        if (written < 0 || written >= (int)(p_end - p)) {
            break;
        }
        p += written;
